...
def generate_models(x, y, degs):
    ...
    models_list = []
    for i in degs:
        models_list.append(np.polyfit(x, y, i))
    return models_list
...
def r_squared(y, estimated):
    ...
    for i in range(len(y)):
        error_est += (y[i] - estimated[i]) ** 2
        error_mean += (y[i] - mean_y) ** 2

    return 1 - (error_est / error_mean)
...
def evaluate_models_on_training(x, y, models):
    ...
    for i in models:
        estYvals = np.polyval(i, x)
        r_squared_val = r_squared(y, estYvals)
        pylab.figure()
        pylab.plot(x, y, 'bo', label='Temperature values')
        pylab.title('Temperature through time')
        pylab.xlabel('Year')
        pylab.ylabel('Temperature')    
        pylab.plot(x, estYvals, 'r-', label = 'R**2 = ' + str(r_squared_val))
        pylab.legend()
        pylab.show()
